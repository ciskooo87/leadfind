...
def fetch_legal_events_from_generic_html(
    url: str,
    item_selector: str,
    title_selector: str,
    ...
    for index, node in enumerate(soup.select(item_selector), start=1):
        ...
        items.append({
            'external_id': (link_node.get('href') if link_node else None) or f'legal-{index}',
...
register_provider('generic_html_legal', fetch_legal_events_from_generic_html)
