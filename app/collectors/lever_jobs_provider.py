...
def _extract_company_from_url(url: str) -> str | None:
    ...
    if path_parts:
        ...
        if first and first not in {'jobs', 'postings', 'careers'}:
            return first.replace('-', ' ') or None
    if host:
        subdomain = host.split('.')[0]
        if subdomain not in {'jobs', 'careers', 'postings'}:
            return subdomain.replace('-', ' ').strip() or None
    return None


def fetch_jobs_from_lever_html(url: str, source_name: str = 'Lever', confidence: float = 0.85) -> list[ProviderJobEvent]:
    html = fetch_text(url)
    ...
    for selector in selectors:
        for node in soup.select(selector):
            ...
            if not href or not text or href in seen_urls:
                continue
            if 'jobs.lever.co/' not in href and '/postings/' not in href:
                continue
            seen_urls.add(href)

            container = node.find_parent(['div', 'section', 'li']) if hasattr(node, 'find_parent') else None
...
register_provider('lever_jobs', fetch_jobs_from_lever_html)
