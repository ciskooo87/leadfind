...
def fetch_regional_news_like_html(url: str) -> list[dict]:
    html = fetch_text(url)
    ...
    for selector in selectors:
        for index, node in enumerate(soup.select(selector), start=1):
            title_node = node.select_one('h1, h2, h3, [data-testid="news-title"]')
            content_node = node.select_one('p, .description, .content, [data-testid="news-description"]')
            ...
            items.append({
                'external_id': href or f'regional-news-{index}',
                'source_url': href,
                'title': title,
                'content': content,
...
register_provider('regional_news_like', fetch_regional_news_like_html)
