...
def infer_signals_from_formal_text(text: str) -> list[tuple[str, str, float]]:
    normalized = normalize_text(text)
    inferred = []
    seen = set()
    for keyword, value in FORMAL_SIGNAL_RULES.items():
...
def normalize_formal_raw_event(db: Session, raw_event: RawEvent) -> RawEvent:
    ...
    if source and source.source_type == 'formal':
        inferred_signals = infer_signals_from_formal_text(f"{raw_event.title or ''} {raw_event.content}")
