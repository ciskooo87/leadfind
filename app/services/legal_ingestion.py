...
from app.collectors.jusbrasil_provider import fetch_jusbrasil_like_html
from app.collectors.legal_html_provider import fetch_legal_events_from_generic_html
...
def _persist_legal_items(db: Session, source: Source, items: list[dict], confidence: float, normalize_after_insert: bool) -> list[RawEvent]:
    ...
    for item in items:
        ...
        if normalize_after_insert:
            raw_event = normalize_legal_raw_event(db, raw_event)
...
def collect_generic_html_legal(db: Session, payload: GenericHtmlLegalCollectRequest) -> list[RawEvent]:
    seed_sources(db)
    ...
    items = fetch_legal_events_from_generic_html(
        url=str(payload.url),
    ...
    return _persist_legal_items(db, source, items, payload.confidence, payload.normalize_after_insert)


def collect_jusbrasil_like(db: Session, payload: JusBrasilCollectRequest) -> list[RawEvent]:
    seed_sources(db)
    source = db.query(Source).filter(Source.name == payload.source_name).first()
    if not source:
        raise ValueError(f'Source not found: {payload.source_name}')

    items = fetch_jusbrasil_like_html(str(payload.url))
    return _persist_legal_items(db, source, items, payload.confidence, payload.normalize_after_insert)
