...
from app.data.reputation_taxonomy import REPUTATION_SIGNAL_RULES
...
def infer_signals_from_reputation_text(text: str) -> list[tuple[str, str, float]]:
    normalized = normalize_text(text)
    inferred: list[tuple[str, str, float]] = []
    seen: set[str] = set()
    for keyword, value in REPUTATION_SIGNAL_RULES.items():
...
def normalize_reputation_raw_event(db: Session, raw_event: RawEvent) -> RawEvent:
    ...
    if source and source.source_type == 'reputation':
        inferred_signals = infer_signals_from_reputation_text(f"{raw_event.title or ''} {raw_event.content}")
