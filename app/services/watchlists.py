...
def run_watchlist(db: Session, watchlist: Watchlist) -> WatchlistRunResult:
    ...
    try:
        ...
        run_log.created_events = len(created_events)
        run_log.generated_leads = generated_leads
        run_log.impacted_company_ids_json = json.dumps(impacted_company_ids)
        run_log.detail = f'Watchlist {watchlist.name} executada com sucesso'
