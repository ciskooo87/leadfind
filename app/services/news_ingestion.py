...
from app.collectors.regional_news_provider import fetch_regional_news_like_html
...
def _persist_news_items(db: Session, source: Source, items: list[dict], confidence: float, normalize_after_insert: bool) -> list[RawEvent]:
...
def collect_generic_html_news(db: Session, payload: GenericHtmlNewsCollectRequest) -> list[RawEvent]:
    ...
    return _persist_news_items(db, source, items, payload.confidence, payload.normalize_after_insert)


def collect_regional_news_like(db: Session, payload: RegionalNewsCollectRequest) -> list[RawEvent]:
    seed_sources(db)
    source = db.query(Source).filter(Source.name == payload.source_name).first()
    if not source:
        raise ValueError(f'Source not found: {payload.source_name}')

    items = fetch_regional_news_like_html(str(payload.url))
    return _persist_news_items(db, source, items, payload.confidence, payload.normalize_after_insert)
