...
from app.data.serasa_taxonomy import SERASA_SIGNAL_RULES
...
def infer_signals_from_serasa_text(text: str) -> list[tuple[str, str, float]]:
    normalized = normalize_text(text)
    inferred = []
    seen = set()
    for keyword, value in SERASA_SIGNAL_RULES.items():
...
def normalize_serasa_raw_event(db: Session, raw_event: RawEvent) -> RawEvent:
    ...
    if source and source.source_type == 'credit':
        inferred_signals = infer_signals_from_serasa_text(f"{raw_event.title or ''} {raw_event.content}")
